...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
  ...
  // Center the grid so dots are visually balanced
  float cellsX = GRID_SIZE;
  float cellsY = GRID_SIZE * ar;
  vec2 numDots = vec2(floor(cellsX + 0.5), floor(cellsY + 0.5));
  vec2 gridOffset = (vec2(cellsX, cellsY) - numDots) * 0.5;
  vec2 uv1 = uv * GRID_SIZE - gridOffset;
  vec2 uv_i = floor(uv1);
  vec2 uv_f = fract(uv1);

  vec2 cellCenter = (uv_i + 0.5 + gridOffset) * gridSizeInverse;
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
  ...
  float dots = 1.0 - step(0.05, distance(uv_f, vec2(0.5)));

  float mask = max(shapeMask, dots);
  vec3 col = mix(u_bg, u_fg, mask);
...
const PingPongScene = ({
  ...
  const uniforms = useMemo<PingPongUniforms>(
    () => ({
      ...
      u_bg: { value: new THREE.Vector3(6 / 255, 6 / 255, 7 / 255) },
      u_fg: { value: new THREE.Vector3(1, 1, 1) },
    }),
  ...
  useEffect(() => {
    uniforms.u_bg.value.copy(bgColor);
    uniforms.u_fg.value.copy(fgColor);
  }, [uniforms, bgColor, fgColor]);
  ...
  useEffect(() => {
    ...
    gl.setClearColor(new THREE.Color(bgColor.x, bgColor.y, bgColor.z), 1);
  ...
  }, [gl, size.width, size.height, uniforms, bgColor]);
...
const DARK_BG = new THREE.Vector3(6 / 255, 6 / 255, 7 / 255);
const DARK_FG = new THREE.Vector3(250 / 255, 250 / 255, 250 / 255);
const LIGHT_BG = new THREE.Vector3(1, 1, 1);
const LIGHT_FG = new THREE.Vector3(9 / 255, 9 / 255, 11 / 255);

const Shader8 = ({ className }: Shader8Props) => {
  const uniformsRef = useRef<PingPongUniforms | null>(null);
  const { resolvedTheme } = useTheme();

  const bgColor = resolvedTheme === "dark" ? DARK_BG : LIGHT_BG;
  const fgColor = resolvedTheme === "dark" ? DARK_FG : LIGHT_FG;
  ...
  return (
    ...
    >
      <Canvas gl={{ alpha: true }} style={{ background: "var(--background)" }}>
        <PingPongScene uniformsRef={uniformsRef} bgColor={bgColor} fgColor={fgColor} />
      </Canvas>
    </section>
  );
};
